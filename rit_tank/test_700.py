...
class Version700Tests(unittest.TestCase):
    ...
    def test_app_version_is_22_00(self):
        self.assertEqual(app.APP_VERSION, '22.00')
    
    def test_config_yaml_version_is_22_00(self):
        config_path = Path(__file__).with_name('config.yaml')
        if config_path.exists():
            content = config_path.read_text()
            self.assertIn("version: '22.00'", content)
    
    def test_readme_title_has_22_00(self):
        readme_path = Path(__file__).with_name('README.md')
        if readme_path.exists():
            content = readme_path.read_text()
            self.assertIn('# Rit & Tank 22.00', content)
