...
function useKnownPlaceGpsOnly(){if(!KNOWN_EDIT_LOCATION){toast('Huidige locatie kon niet worden opgehaald.',true);return}selectKnownPlaceLocation(KNOWN_EDIT_LOCATION);$('knownCoord').textContent=`Geselecteerde GPS-positie: ${fmt(KNOWN_EDIT_LOCATION.latitude,5)} · ${fmt(KNOWN_EDIT_LOCATION.longitude,5)}`}
function editKnownPlace(id){let p=(DATA?.business?.known_places||[]).find(x=>Number(x.id)===Number(id));if(!p)return;$('knownId').value=p.id;$('knownName').value=p.name||'';$('knownCategory').value=p.category||'other';$('knownArrival').value=p.arrival_trip_type||'ask';$('knownDepart').value=p.unknown_departure_trip_type||'ask';$('knownRadius').value=p.radius_m||180;$('knownRadiusVal').textContent=(p.radius_m||180)+' m';$('knownLat').value=p.latitude;$('knownLon').value=p.longitude;$('knownCoord').textContent=`📍 ${fmt(p.latitude,5)}, ${fmt(p.longitude,5)}`;KNOWN_EDIT_LOCATION={latitude:p.latitude,longitude:p.longitude,address:''};$('knownAddressChoices').innerHTML='';$('knownAddressResults').innerHTML='';$('knownAddressSearch').hidden=true;$('knownAddressQuery').value='';$('knownGpsOnly').hidden=true;$('knownEditTitle').textContent='📌 '+p.name;closeModal('knownPlacesModal');openModal('knownPlaceEditModal')}
