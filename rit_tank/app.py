...
def get_settings() -> dict[str, Any]:
    ...
    out = {
        ...
        'km_reimbursement_rate': str(opts.get('km_reimbursement_rate', 0.25)),
        'driver_name': '',
        'company_name': '',
        'vehicle_make': '',
        'vehicle_model': '',
...
def home_destination() -> dict[str, Any]:
    """Resolve the fixed Home shortcut without creating or changing a known place."""
    target = ' '.join(HOME_ADDRESS.split()).casefold()
    for place in known_places_all():
        name = str(place.get('name') or '').strip().casefold()
        address = ' '.join(str(place.get('address') or '').split()).casefold()
        coords = _valid_coordinates(place.get('latitude'), place.get('longitude'))
        if name == 'thuis' and address == target and coords:
            return {
                'address': HOME_ADDRESS,
                'latitude': coords[0],
                'longitude': coords[1],
                'place_id': str(place.get('place_id') or ''),
                'known_place_id': place.get('id'),
                'source': 'known_place',
            }

    places = google_places_text_search(HOME_ADDRESS)
    for place in places:
        address = ' '.join(str(place.get('address') or '').split()).casefold().rstrip(' ,.')
        if address != target and not address.startswith(target + ','):
            continue
        coords = _valid_coordinates(place.get('latitude'), place.get('longitude'))
        if not coords:
            continue
    ...
    raise ValueError(f'Google Places kon {HOME_ADDRESS} niet met geldige coördinaten vinden.')


def preview_business_destination(payload: dict[str, Any]) -> dict[str, Any]:
    """Preview the selected active-trip destination using the existing route helper."""
    trip = active_business_trip()
    if not trip or not trip.get('stops'):
        raise ValueError('Er is geen actieve ritregistratie.')
    destination = _valid_coordinates(payload.get('latitude'), payload.get('longitude'))
    if not destination:
        raise ValueError('Geen geldige bestemmingscoördinaten ontvangen.')
    last = trip['stops'][-1]
    origin = _valid_coordinates(last.get('latitude'), last.get('longitude'))
    if not origin:
        raise ValueError('Het vorige adres heeft geen geldige coördinaten.')
    route = get_route_distance(origin[0], origin[1], destination[0], destination[1])
    distance_m = to_float(route.get('distance_m'))
    source = route.get('type') if route.get('type') in {'route', 'gps'} else 'gps'
    base = to_float(last.get('odometer'))
    calibration = distance_calibration()
    suggested = None
    if base is not None and distance_m is not None and distance_m > 0:
        factor = 1.0 if source == 'route' else float(calibration.get('factor') or 1.0)
        suggested = round(base + distance_m / 1000.0 * factor)
    return {
...
<div class="modal" id="assistantModal"><div class="sheet"><div class="grab"></div><div class="sheethead"><h2>📍 Je bent aangekomen</h2><button class="close" onclick="closeModal('assistantModal')">✕</button></div>
  ...
  <div class="guide-section" id="assistantRouteSection">
   ...
   <div class="assistant-actions"><button class="odo-suggest-edit" type="button" onclick="searchAssistantAddress()">🔍 Zoeken</button></div>
...
<div class="modal" id="settingsModal"><div class="sheet"><div class="grab"></div><div class="sheethead"><h2>⚙️ Instellingen</h2><button class="close" onclick="closeModal('settingsModal')">✕</button></div>
  ...
  <div class="field"><label for="setKmRate">Zakelijke kilometervergoeding</label><div style="display:flex;align-items:center;gap:8px"><span>€</span><input id="setKmRate" type="text" inputmode="decimal" aria-describedby="setKmRateHelp"><span style="white-space:nowrap">per km</span></div><small id="setKmRateHelp">Dit bedrag wordt gebruikt voor de zakelijke kilometerdeclaratie.</small></div>
...
async function searchTripManualAddress(){
  let query=$('tripManualAddress').value.trim(),request=++TRIP_ADDRESS_REQUEST;
  clearTimeout(TRIP_SEARCH_TIMER);
  if(!query)return;
  try{
    ...
    if(request!==TRIP_ADDRESS_REQUEST)return;
    let places=result.places||[],box=$('tripAddressChoices');box.innerHTML='';
    $('tripGoogleAttrib').style.display=places.length?'block':'none';
    if(!places.length){box.innerHTML='<div class="empty">Geen adressen gevonden.</div>';return}
    places.slice(0,10).forEach(p=>{
      let b=document.createElement('button');b.type='button';b.className='guide-next';b.textContent=p.address||p.name||'Adres';
      b.onclick=()=>{
        let label=p.address||p.name||query;
        if(p.latitude!=null&&p.longitude!=null&&Number.isFinite(Number(p.latitude))&&Number.isFinite(Number(p.longitude))){
          confirmTripAddress(label,{...p,latitude:Number(p.latitude),longitude:Number(p.longitude),source:'provider_confirmed'});
        }else{toast('Dit adres heeft geen geldige coördinaten.',true)}
      };box.appendChild(b);
    });
  }catch(e){if(request===TRIP_ADDRESS_REQUEST)toast(e.message,true)}
}
function advanceTripAfterLocation(){if(TRIP_LOCATION)guideTo('tripSaveButton',80)}
async function confirmTripAddress(label,location){
  clearTimeout(TRIP_SEARCH_TIMER);++TRIP_ADDRESS_REQUEST;
  $('tripManualAddress').value=label;
  TRIP_LOCATION={...location,place_id:location.place_id||'',address:label,manual_label:label};
  TRIP_SEGMENT_TYPE='business';
  let selected=TRIP_LOCATION,request=++TRIP_ROUTE_PREVIEW_REQUEST;
  $('tripLocationTitle').textContent='✓ Adres bevestigd';
  $('tripLocationDetail').textContent=label;
  for(let b of $('tripAddressChoices').children)b.classList.toggle('selected',b.textContent.startsWith(label+' ·'));
  advanceTripAfterLocation();
  if(TRIP_MODE==='start')return;
  try{
    let preview=await api('api/business/route-preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({latitude:selected.latitude,longitude:selected.longitude})});
    if(TRIP_LOCATION!==selected||request!==TRIP_ROUTE_PREVIEW_REQUEST)return;
    let distance=preview.distance_m==null?null:Number(preview.distance_m);
    let detail=distance==null?'Afstand nog niet beschikbaar':fmt(distance/1000,1)+' km '+(preview.distance_source==='route'?'via wegroute':'GPS-schatting');
    if(preview.suggested_odometer!=null&&Number.isFinite(Number(preview.suggested_odometer))){
      let proposed=Number(preview.suggested_odometer);
      $('tripOdoSuggestedValue').textContent=fmt(proposed,0);
      $('tripOdoSuggestedDetail').textContent='Opnieuw berekend vanaf de vorige locatie ('+detail+'). Controleer de echte tellerstand.';
      if(!TRIP_ODO_MANUAL)initOdometerWheel('trip',proposed);
      $('tripOdoSuggestion').classList.add('show');
      if(!TRIP_ODO_MANUAL)$('tripOdoEditor').hidden=true;
      $('tripOdoStepHint').textContent='Voorstel bijgewerkt voor de geselecteerde bestemming';
      detail+=' · tellerstandsuggestie '+fmt(proposed,0)+' km';
    }
    $('tripLocationDetail').textContent=label+' · '+detail;
  }catch(e){
    if(TRIP_LOCATION===selected&&request===TRIP_ROUTE_PREVIEW_REQUEST)$('tripLocationDetail').textContent=label+' · Afstandsschatting niet beschikbaar';
  }
}
async function selectTripHome(){
  clearTimeout(TRIP_SEARCH_TIMER);
  let button=$('tripHomeButton'),request=++TRIP_ADDRESS_REQUEST;
  if(button)button.disabled=true;
  try{
    let home=await api('api/places/home');
    if(request!==TRIP_ADDRESS_REQUEST)return;
    if(!Number.isFinite(Number(home.latitude))||!Number.isFinite(Number(home.longitude)))throw new Error('Thuis heeft geen geldige coördinaten.');
    await confirmTripAddress('Verenlandweg 4, 7461 AP Rijssen',{...home,latitude:Number(home.latitude),longitude:Number(home.longitude)});
  }catch(e){
    if(request===TRIP_ADDRESS_REQUEST)toast(e.message,true);
  }finally{if(button)button.disabled=false}
}
$('tripManualAddress').addEventListener('input',()=>{
  clearTimeout(TRIP_SEARCH_TIMER);++TRIP_ADDRESS_REQUEST;++TRIP_ROUTE_PREVIEW_REQUEST;TRIP_LOCATION=null;
  $('tripLocationTitle').textContent='Kies een adres uit de zoekresultaten';$('tripLocationDetail').textContent='';
  $('tripAddressChoices').innerHTML='';$('tripGoogleAttrib').style.display='none';
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == '/api/places/home':
            try:
                return json_response(self, home_destination())
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == '/api/business/route-preview':
                return json_response(self, preview_business_destination(payload))
