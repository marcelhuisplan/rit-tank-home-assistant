"""Release 18.00 PDF regression tests: one row per leg and business reimbursement output."""
import re
import unittest
...
MULTI_STOP = (
    dict(purpose='Drie stops', status='completed', trip_type='business'),
    [
        ...
        dict(created_at='2026-03-02T15:45:00+01:00', odometer=63929,
             manual_label='Verenlandweg 4, 7461 AP Rijssen', segment_trip_type='business'),
    ],
)

TWO_STOP = (
    dict(purpose='Twee stops', status='completed', trip_type='business'),
    [
...
UNCLASSIFIED_MIXED = (
    dict(purpose='Gemengd segment', status='completed', trip_type='business'),
    [
...
class PdfLegExportTests(unittest.TestCase):
    ...
    def test_multi_stop_trip_exports_every_leg(self):
        ...
        self.assertEqual(rows.count('€ 0,50'), 2)
    ...
    def test_two_stop_trip_stays_one_row(self):
        ...
        self.assertEqual(rows.count('€ 2,50'), 1)
    ...
    def test_unclassified_leg_still_exports_without_type_column(self):
        ...
        self.assertIn('10,0 km', texts)
        self.assertIn('€ 2,50', texts)
        self.assertNotIn('Privé/Zakelijk', texts)
        self.assertNotIn('Zakelijk', texts)
        self.assertNotIn('Privé', texts)
...
class PdfReimbursementGeometryTests(unittest.TestCase):
    def test_reimbursement_header_is_right_aligned(self):
        self.assertGreater(REIMBURSEMENT_RIGHT, ODOMETER_X)
        text_end = REIMBURSEMENT_RIGHT - _table_text_width('Vergoeding', 9.5, bold=True)
        self.assertGreater(text_end, ODOMETER_X)

    def test_reimbursement_does_not_overlap_distance_column(self):
        reimbursement_w = _table_text_width('€ 1.234,56', 9.5, bold=True)
        distance_w = _table_text_width('1234,5 km', 9.5, bold=True)
        self.assertLess(REIMBURSEMENT_RIGHT, 557 - distance_w + reimbursement_w)

    def test_address_column_stops_before_badge(self):
        self.assertLess(ADDRESS_X + ADDRESS_MAX_WIDTH, ODOMETER_X)

    def test_pages_show_reimbursement_header_and_no_type_badges(self):
        ...
        pages = app.business_pdf('month', '2026', '3', preview=True)['pages']
        for markup in pages:
            self.assertIn('Vergoeding', _texts(markup))
            self.assertNotIn('Soort', _texts(markup))
